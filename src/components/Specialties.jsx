...
export default function Specialties() {
  ...
  return (
    <section
      ref={sectionRef}
      id="especialidades"
      className="py-16 lg:py-24 bg-[#F4F7FA]"
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 lg:mb-12 gap-6">
          <div>
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0B2A4A] tracking-tight">
              Cuidado Dental Completo <br className="hidden sm:inline" />
              para{" "}
          ...
          {/* BOTÃO AGENDAR */}
          <button 
            onClick={() => handleWhatsApp("Avaliação Geral")}
            className="inline-flex items-center gap-3 text-xs font-bold text-[#0B2A4A] hover:text-[#164A73] transition-colors uppercase tracking-wider group cursor-pointer self-start md:self-auto"
          >
        ...
        {/* GRID VERTICAL (1 Coluna no Mobile, 2 no Tablet, 3 no Desktop) */}
          ...
          {specialties.map((item, index) => {
            ...
            return (
              ...
              >
                <div>
                  {/* Container do Ícone */}
                  <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-[#F0F5FA] group-hover:bg-[#0B2A4A] text-[#164A73] group-hover:text-white flex items-center justify-center mb-6 sm:mb-8 transition-all duration-300 shadow-inner">
                    <IconComponent className="w-6 h-6 sm:w-7 sm:h-7 stroke-[1.5]" />
                  </div>

                  {/* Título do Serviço */}
                  <h3 className="text-lg sm:text-xl font-bold text-[#0B2A4A] tracking-tight mb-2 sm:mb-3 group-hover:text-[#164A73] transition-colors">
                    {item.title}
                  </h3>
